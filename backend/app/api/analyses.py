...
from app.dependencies import get_repository, require_standard_rate_limit
...
async def list_analyses(
    ...
    result = await repository.list(user.id, page, page_size)
...
async def analysis_stats(
    ...
    return SuccessResponse(data=await repository.stats(user.id), meta=ResponseMeta(request_id=request.state.request_id))
...
async def get_analysis(
    ...
    result = await repository.get(analysis_id, user.id)
    if result is None:
        raise AppError("Analysis not found.", 404, "analysis_not_found")
    return SuccessResponse(data=result, meta=ResponseMeta(request_id=request.state.request_id))
